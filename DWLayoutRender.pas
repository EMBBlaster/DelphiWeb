...
procedure TDWLayoutRender.ProcessFormAsync(aForm: TDWCustomForm);
  procedure ProcessChildControls(AControl: TControl);
  var
   // CurEl: TDWElementTag;
    C: Integer;
    L_IControl:IDWControl;
  begin
    if Supports(AControl, IDWControl, L_IControl)
    ...
    if csAcceptsControls in AControl.ControlStyle then
      begin
        ...
        for C := 0 to (AControl as TDWContainer).ControlCount - 1 do
          begin
            ProcessChildControls((AControl as TDWContainer).Controls[C]);
          end;
      end;
  end;
...
begin
  ...
  for I := 0 to aForm.ControlCount - 1 do
    begin
      ProcessChildControls(aForm.Controls[I]);
    end;
end;

procedure TDWLayoutRender.ProcessChildControls(aControl: TControl;
  aParentElement: TDWElementTag);
var
    CurEl: TDWElementTag;
    C: Integer;
    L_IControl:IDWControl;
        ...
        if AControl.InheritsFrom(TDWInputControl) then
          begin
            ...
            if AControl.Parent.InheritsFrom(TDWCustomForm) then
              ...
              LForm.HTMLPage.HTMLTag.HeadTag.InitComponents.Add(
                                  'DWInitControl(DWAjaxForm, "' +
              ...
              and (Supports(AControl.Parent, IDWControl, L_IPArent)) then
                ...
                LForm.HTMLPage.HTMLTag.HeadTag.InitComponents.Add('DWInitControl(' +
                    L_IPArent.HTMLName + ', "' + (AControl as TDWControl).Name +
                    '", true, true);');
          end;
